-- Update create_public_user function to handle existing users
...
DECLARE
  new_user JSONB;
BEGIN
  -- First, delete any existing user with the same ID or email
  DELETE FROM public.users WHERE id = user_id OR email = user_email;
...
-- Comment on function
COMMENT ON FUNCTION create_public_user IS 'Creates or replaces a user record in public.users bypassing RLS policies. Used by the Chrome extension.';
