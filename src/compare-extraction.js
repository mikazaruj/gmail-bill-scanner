...
async function main() {
  try {
    ...
    if (!pdfPath) {
      ...
      console.log('Usage: node src/compare-extraction.js path/to/pdf/file.pdf');
      process.exit(1);
    }
 ...
 */
async function compareFullBillExtraction(pdfData, simpleText, regularText, fileName) {
  try {
    const matcher = new UnifiedPatternMatcher();
    
    // Extract with regular PDF approach
    console.log('\n[COMPARE] Regular PDF extraction result:');
    const regularResult = await matcher.extract({
      pdfData,
      fileName
    }, {
      language: 'hu',
    ...
    const debugResult = await matcher.extract({
      text: simpleText,
      fileName
    }, {
      language: 'hu',
